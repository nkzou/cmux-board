...
// deepCopyState returns a deep copy of s by marshaling to JSON and back. This
// ensures full isolation of all maps, slices, and pointer fields. Nil maps are
// re-initialized after the round-trip so callers can always assign into them
// without a nil-map panic. Panics on marshal/unmarshal failure, which indicates
// programmer error (a State with non-serializable values should never exist).
