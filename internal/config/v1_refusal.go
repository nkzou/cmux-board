...
// readSchemaVersion reads schema_version from the JSON file at path. exists is
// false when the file is absent (first-run case), in which case version is 0 and
// err is nil.
func readSchemaVersion(path string) (version int, exists bool, err error) {
	...
	if err != nil {
		...
		return 0, false, fmt.Errorf("failed to read file for schema check: %w", err)
	}
	var sv schemaVersionOnly
	if err := json.Unmarshal(data, &sv); err != nil {
		return 0, true, fmt.Errorf("failed to parse schema_version from %s: %w", path, err)
	}
	return sv.SchemaVersion, true, nil
}
...
func CheckSchemaVersion(path string) error {
	version, _, err := readSchemaVersion(path)
	if err != nil {
		return err
	}
	if version == 1 {
...
func checkFileSchemaV2(path string) error {
	version, exists, err := readSchemaVersion(path)
	if err != nil {
		return err
	}
	if exists && version < 2 {
