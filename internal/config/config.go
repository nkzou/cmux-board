...
// DeriveRepoID computes the default repo_id from a display name by slugifying it
// (lowercase, non-[a-z0-9-] runs collapsed to '-', ends trimmed). Returns "" for
// names with no slug-able characters; callers reject an empty result.
func DeriveRepoID(displayName string) string {
	return tracker.Slug(displayName)
}

// DefaultStarterPromptTemplate is the default value for config.json
// "claude.starter_prompt". Available template variables: {{.Ticket.*}},
// {{.Repo.*}}, {{.WorktreePath}}, {{.ApproachName}}.
