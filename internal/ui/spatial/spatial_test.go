...
import (
	...
	"github.com/nkzou/cmux-board/internal/ui/spatial"
)
...
func TestClamp(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			x, y := spatial.Clamp(tt.x, tt.y, tt.cardW, tt.cardH, tt.boardW, tt.boardH)
...
func TestApplyDelta(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			x, y := spatial.ApplyDelta(tt.startX, tt.startY, tt.cursorStartX, tt.cursorStartY, tt.cursorNowX, tt.cursorNowY)
			if x != tt.wantX || y != tt.wantY {
				t.Errorf("got (%d,%d), want (%d,%d)", x, y, tt.wantX, tt.wantY)
			}
		})
...
func TestHitTest_NoOverlap(t *testing.T) {
	t.Parallel()
	cards := []spatial.Positioned{
		{ID: "A", X: 0, Y: 0, W: 5, H: 3},
		{ID: "B", X: 10, Y: 10, W: 5, H: 3},
	}
	zOrder := []string{"A", "B"}
	id, found := spatial.HitTest(cards, zOrder, 20, 20)
	if found {
...
func TestHitTest_TwoOverlapping_LastOnTopWins(t *testing.T) {
	...
	id, found := spatial.HitTest(cards, zOrder, 3, 2)
	if !found || id != "B" {
...
func TestZOrder_CyclesLastInteractedToTop_PreservesRest(t *testing.T) {
	...
	result := spatial.ZOrder(current, "B")
	want := []string{"A", "C", "D", "B"}
...
func TestZOrder_MissingKey_Unchanged(t *testing.T) {
	...
	result := spatial.ZOrder(current, "X")
...
func TestZOrder_AlreadyOnTop_Unchanged(t *testing.T) {
	t.Parallel()
	current := []string{"A", "B", "C"}
	result := spatial.ZOrder(current, "C")
...
func TestHitTest_EmptyCards(t *testing.T) {
	t.Parallel()
	_, found := spatial.HitTest(nil, nil, 5, 5)
	if found {
